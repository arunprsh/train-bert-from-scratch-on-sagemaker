...
if __name__ == '__main__':
    ...
    # Download saved original BERT vocabulary file from S3 to local input path of the training cluster
    logger.info(f'Downloading custom vocabulary from [{S3_BUCKET}/data/bert/vocab/] to [{args.input_dir}/vocab/]')
    ...
    with open(f'{path}/vocab.txt', 'wb') as file_:
    ...
    if current_host == master_host:
        ...
        if os.path.exists('/tmp/cache/model/finetuned/pytorch_model.bin') and os.path.exists('/tmp/cache/model/finetuned/config.json'):
            ...
            s3.meta.client.upload_file('/tmp/cache/model/finetuned/pytorch_model.bin', S3_BUCKET, 'model/finetuned/pytorch_model.bin')
